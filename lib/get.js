...
export async function getEquipmentsTypes(userId) {
  try {
...
export async function getInspectionsTypes() {
  try {
    const result = await excuteQuery({
      query: "SELECT * FROM inspections_reports WHERE user_id = ?",
      values: [1],
    });
    console.log(result);
    return result;
        ...
        for (const inspectionType of result) {
          try {
            var secondResult = await excuteQuery({
              query:
                "SELECT * FROM inspections_reports WHERE id = ?",
              values: [inspectionType.inspection_report_id],
            });
...
export async function getInspectionsFormQuestion(id) {
  try {
    const result = await excuteQuery({
      query: "SELECT * FROM inspections_options WHERE inspection_report_id = ?",
      values: [id],
    });
    console.log(result);
    return result;
...
export async function getInspections(equipmentId) {
  try {
    const result = await excuteQuery({
      query: "SELECT * FROM inspections WHERE equipment_id = ?",
      values: [equipmentId],
    });
    console.log(result);
    return result;
