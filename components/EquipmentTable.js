...
function EquipmentTable({ data }) {
  ...
  const handleDelete = (id, serialNb) => {
  ...
  const confirmDelete = () => {
    ...
    const deleteEquipment = async () => {
      let serverResponse = await fetch(
        "/api/equipments/" + equipmentToDeleteId,
        {
          method: "DELETE",
          headers: {
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {data.map((type) => (
                    <tr key={type.serial_nb}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <a
                          onClick={() => handleDelete(type.id, type.serial_nb)}
      ...
      {showCard && (
        <>
          <DeleteCard
            ...
            message={"Voulez-vous vraiment supprimer l'équipement " + equipmentToDeleteName}
          />
