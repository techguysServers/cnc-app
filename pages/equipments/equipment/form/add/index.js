...
function AddEquipment({ equipmentTypeData, equipmentTypeOptions }) {
  const router = useRouter();
  const { id } = router.query;
  const [optionData, setOptionData] = useState(equipmentTypeOptions);
...
export async function getServerSideProps(context) {
  ...
  let data = await serverResponse.json();
  console.log(data);
  let equipmentTypeOptions = await getRequest("get?request=equipment_types_options&id=" + id)
  console.log(equipmentTypeOptions)

  return {
    props: {
      equipmentTypeData: data.data[0],
      equipmentTypeOptions: equipmentTypeOptions.data
    },
  };
}
